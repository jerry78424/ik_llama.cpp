...
#define LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY 1
   ...
   // Copy DFlash2 selector lattice after a draft decode with top_k*top_k scores
    ...
    // Get the DSpark confidence head value (sigmoid, in [0,1]) for draft position i
    // without materializing full logits. Returns -1.0f when unavailable.
    LLAMA_API float llama_get_dflash_dspark_conf_ith(struct llama_context * ctx, int32_t i);
